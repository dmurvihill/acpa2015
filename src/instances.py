...
def start_instance(
        ...
        type='t2.micro', placement=None):
    ''' Start an EC2 instance using the default settings that we care about '''
    return start_instances(1,type,placement)[0]

def start_instances(n, ec2=boto.ec2.connect_to_region('us-west-2'),
        type='t2.micro', zone=None):
...
def terminate_instance(id, ec2=boto.ec2.connect_to_region('us-west-2')):
